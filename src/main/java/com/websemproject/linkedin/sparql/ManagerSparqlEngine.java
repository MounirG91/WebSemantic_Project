...
//		 + "\\WebSemantic_Project";
		...
		System.out.println (get_AllSkills ());
		System.out.println (get_AllLanguages ());
		System.out.println (get_AllIndustries ());
		System.out.println (get_AllLanguageCertifs ());
		System.out.println (get_AllITCertifs ());
		System.out.println (get_AllEducations ());
		System.out.println (get_AllCurrentPositions ());
		System.out.println (get_AllPastPositions ());
		System.out.println (get_AllCountries ());
		System.out.println (get_AllUniversities ());
		System.out.println (get_AllCompanies ());
		...
		List<String> indus = new ArrayList ();
		indus.add("Computer Software");
		indus.add("Computer-Software");
	
		List<String> langcertifs = new ArrayList ();
		langcertifs.add("TOEIC");
		
		List<String> itcertifs = new ArrayList ();
		itcertifs.add("LPIC1");
		itcertifs.add("SCJP");

		
		executeQuery(skills, lang,
				indus, langcertifs, itcertifs ,
				null, null, null,
				null, null, null,
				null, null);
	...
	public static List<String> get_AllSkills() {
	...
	public static List<String> get_AllLanguages() {
	...
	public static List<String> get_AllIndustries() {
	...
	public static List<String> get_AllLanguageCertifs() {
	...
	public static List<String> get_AllITCertifs() {
	...
	public static List<String> get_AllEducations() {
	...
	public static List<String> get_AllCurrentPositions() {
	...
	public static List<String> get_AllPastPositions() {
	...
	public static List<String> get_AllCountries() {
		...
		for (Mapping m : map) {
			IDatatype dt = (IDatatype) m.getValue("?y");
			if (dt.stringValue().substring(dt.stringValue().indexOf("#")+1, dt.stringValue().length()).equalsIgnoreCase("Deutschland")
			|| dt.stringValue().substring(dt.stringValue().indexOf("#")+1, dt.stringValue().length()).equalsIgnoreCase("Allemagne"))
				list.add("Germany");

			else if  (dt.stringValue().substring(dt.stringValue().indexOf("#")+1, dt.stringValue().length()).equalsIgnoreCase("Tunisie"))
				list.add("Tunisia");
			else
				list.add(dt.stringValue().substring(dt.stringValue().indexOf("#")+1, dt.stringValue().length()));
		}
	...
	public static List<String> get_AllUniversities() {
	...
	public static List<String> get_AllCompanies() {
	...
	public static List<String> executeQuery(List<String> skills, List<String> languages,
			List<String> industries, List<String> languagesCertifs, List<String> itCertifs, 
			List<String> educations, List<String> currentPost, List<String> pastPost,
			List<String> countries, List<String> universities, List<String> companies,
...
//Skills prepare query
		...
		query_skills = "PREFIX lp: <http://linkedin_project.com/profile.rdfs#> "+
       "select  ?x "+
       "where { "+
       "?x lp:hasSkill ?y"+
      " FILTER ("+sub_query_skills+")"+
      " }";
		}
...
//Languages prepare query		
		...
		String sub_query_lang	 ="";	
		 ...
		 query_lang = "PREFIX lp: <http://linkedin_project.com/profile.rdfs#> "+
		       "select  ?x "+
		       "where { "+
		       "?x lp:speacksLanguage ?y"+
		      " FILTER ("+sub_query_lang+")"+
		      " }";
		}
...
//Industries prepare query		
		...
		if (industries != null) 
		{
		 ...
		 query_ind = "PREFIX lp: <http://linkedin_project.com/profile.rdfs#> "+
		       "select  ?x "+
		       "where { "+
		       "?x lp:industry ?y"+
		      " FILTER ("+sub_query_ind+")"+
		      " }";
...
//langCertifs prepare query		
				...
				if (languagesCertifs != null) 
				{
						...
						for (int i = 0; i < languagesCertifs.size(); i++) {
							sub_query_lang_certif += "str(?z) = \""+uri+"certificationInfoBox/#" + 
									languagesCertifs.get(i)+"\"";
							if (i!=(languagesCertifs.size()-1))
								sub_query_lang_certif += " || ";	
						}
						
						query_lang_certif = "PREFIX lp: <http://linkedin_project.com/profile.rdfs#> "+
				       "select  ?x "+
				       "where { "+
				       "?x lp:hasCertification ?y "+
				       "?y rdf:type \"" +base + "languageCertification\" " +
				       "?y lp:hasInfoBox ?z "+
				       " FILTER ("+sub_query_lang_certif+")"+
				      " }";
	...
	if (itCertifs != null) 
				{
						...
						for (int i = 0; i < itCertifs.size(); i++) {
							sub_query_it_certif += "str(?z) = \""+uri+"certificationInfoBox/#" + 
									itCertifs.get(i)+"\"";
							if (i!=(itCertifs.size()-1))
								sub_query_it_certif += " || ";	
						}
						
						query_it_certif = "PREFIX lp: <http://linkedin_project.com/profile.rdfs#> "+
				       "select  ?x "+
				       "where { "+
				       "?x lp:hasCertification ?y "+
				       "?y rdf:type \"" +base + "ITCertification\" " +
				       "?y lp:hasInfoBox ?z "+
				       " FILTER ("+sub_query_it_certif+")"+
				      " }";
